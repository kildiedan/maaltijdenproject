...
var build = require('./dep-bc4956ea.js');
var fs$3 = require('fs');
...
function formatFrom$1(source) {
  ...
  if (typeof source == 'object') {
    return override$6(DEFAULTS$2, source);
  }
  ...
  if (typeof source == 'string') {
    return override$6(DEFAULTS$2, toHash(source));
  }
...
function hasInvalidCharacters(value) {
  ...
  for (i = 0, l = value.length; i < l; i++) {
    ...
    if (isEscaped) ; else if (character == Marker$e.SINGLE_QUOTE || character == Marker$e.DOUBLE_QUOTE) {
      isQuote = !isQuote;
    } else if (!isQuote && (character == Marker$e.CLOSE_CURLY_BRACKET || character == Marker$e.EXCLAMATION || character == LESS_THAN || character == Marker$e.SEMICOLON)) {
      isInvalid = true;
      break;
    ...
    isEscaped = character == Marker$e.BACK_SLASH;
  }
...
function removeWhitespace(value, format) {
  ...
  for (i = 0, l = value.length; i < l; i++) {
    ...
    isNewLineNix = character == Marker$e.NEW_LINE_NIX;
    isNewLineWin = character == Marker$e.NEW_LINE_NIX && value[i - 1] == Marker$e.CARRIAGE_RETURN;
    ...
    } else if (character == Marker$e.OPEN_SQUARE_BRACKET && !isQuoted) {
      stripped.push(character);
      isAttribute = true;
    } else if (character == Marker$e.CLOSE_SQUARE_BRACKET && !isQuoted) {
      stripped.push(character);
      isAttribute = false;
    } else if (character == Marker$e.OPEN_ROUND_BRACKET && !isQuoted) {
      stripped.push(character);
      roundBracketLevel++;
    } else if (character == Marker$e.CLOSE_ROUND_BRACKET && !isQuoted) {
      stripped.push(character);
      roundBracketLevel--;
    } else if (character == Marker$e.SINGLE_QUOTE && !isQuoted) {
      stripped.push(character);
      isSingleQuoted = true;
    } else if (character == Marker$e.DOUBLE_QUOTE && !isQuoted) {
      stripped.push(character);
      isDoubleQuoted = true;
    } else if (character == Marker$e.SINGLE_QUOTE && isQuoted) {
      stripped.push(character);
...
function restoreHack(property) {
  if (property.hack[0] == Hack$2.UNDERSCORE) {
    ...
    property.value[property.value.length - 1][1] += Marker$d.SPACE + BANG_HACK;
  }
}
...
function isMultiplex(property) {
  ...
  for (i = 3, l = property.length; i < l; i++) {
    ...
    if (value[0] == Token$n.PROPERTY_VALUE && (value[1] == Marker$c.COMMA || value[1] == Marker$c.FORWARD_SLASH)) {
      return true;
    }
  }
...
DEFAULTS$1[OptimizationLevel$7.Zero] = {};
DEFAULTS$1[OptimizationLevel$7.One] = {
...
DEFAULTS$1[OptimizationLevel$7.Two] = {
...
function optimizationLevelFrom$1(source) {
  ...
  var Zero = OptimizationLevel$7.Zero;
  var One = OptimizationLevel$7.One;
  var Two = OptimizationLevel$7.Two;
...
function split$3(value, separator) {
  var openLevel = Marker$b.OPEN_ROUND_BRACKET;
  var closeLevel = Marker$b.CLOSE_ROUND_BRACKET;
  var level = 0;
  var cursor = 0;
  var lastStart = 0;
  var lastValue;
...
function afterClosingBrace(token, valueIndex) {
  return token[valueIndex][1][token[valueIndex][1].length - 1] == Marker$a.CLOSE_ROUND_BRACKET;
}

function afterComma(token, valueIndex) {
  return token[valueIndex][1] == Marker$a.COMMA;
}

function afterSlash(token, valueIndex) {
  return token[valueIndex][1] == Marker$a.FORWARD_SLASH;
}

function beforeComma(token, valueIndex) {
  return token[valueIndex + 1] && token[valueIndex + 1][1] == Marker$a.COMMA;
}

function beforeSlash(token, valueIndex) {
  return token[valueIndex + 1] && token[valueIndex + 1][1] == Marker$a.FORWARD_SLASH;
}
...
function lastPropertyIndex(tokens) {
  ...
  for (; index >= 0; index--) {
    if (tokens[index][0] != Token$m.COMMENT) {
      break;
    }
  }
...
function property$1(context, tokens, position, lastPropertyAt) {
  ...
  var isPropertyBlock = propertyValue && propertyValue[0] === Token$m.PROPERTY_BLOCK;
  ...
  switch (token[0]) {
    case Token$m.AT_RULE:
      store(context, token);
      store(context, semicolon(context, Breaks.AfterProperty, false));
      break;
    case Token$m.AT_RULE_BLOCK:
      rules$1(context, token[1]);
      store(context, openBrace(context, Breaks.AfterRuleBegins, true));
      body$1(context, token[2]);
      store(context, closeBrace(context, Breaks.AfterRuleEnds, false, isLast));
      break;
    case Token$m.COMMENT:
      store(context, token);
      break;
    case Token$m.PROPERTY:
...
function value$1(context, token) {
  ...
  if (token[2][0] == Token$m.PROPERTY_BLOCK) {
...
function openBrace(context, where, needsPrefixSpace) {
  if (context.format) {
    ...
    return (needsPrefixSpace && allowsSpace(context, Spaces.BeforeBlockBegins) ? Marker$a.SPACE : emptyCharacter) +
      Marker$a.OPEN_CURLY_BRACKET +
      (allowsBreak(context, where) ? context.format.breakWith : emptyCharacter) +
      context.indentWith;
  } else {
    return Marker$a.OPEN_CURLY_BRACKET;
  }
}
...
function closeBrace(context, where, beforeBlockEnd, isLast) {
  if (context.format) {
    ...
    return (allowsBreak(context, Breaks.AfterProperty) || beforeBlockEnd && allowsBreak(context, Breaks.BeforeBlockEnds) ? context.format.breakWith : emptyCharacter) +
      context.indentWith +
      Marker$a.CLOSE_CURLY_BRACKET +
      (isLast ? emptyCharacter : (allowsBreak(context, where) ? context.format.breakWith : emptyCharacter) + context.indentWith);
  } else {
    return Marker$a.CLOSE_CURLY_BRACKET;
  }
}

function colon(context) {
  return context.format ?
    Marker$a.COLON + (allowsSpace(context, Spaces.BeforeValue) ? Marker$a.SPACE : emptyCharacter) :
    Marker$a.COLON;
}

function semicolon(context, where, isLast) {
  return context.format ?
    Marker$a.SEMICOLON + (isLast || !allowsBreak(context, where) ? emptyCharacter : context.format.breakWith + context.indentWith) :
    Marker$a.SEMICOLON;
}

function comma(context) {
  return context.format ?
    Marker$a.COMMA + (allowsBreak(context, Breaks.BetweenSelectors) ? context.format.breakWith : emptyCharacter) + context.indentWith :
    Marker$a.COMMA;
}
...
function optimizeColors(name, value, compatibility) {
  ...
  if (compatibility.colors.opacity && name.indexOf('background') == -1) {
    value = value.replace(/(?:rgba|hsla)\(0,0%?,0%?,0\)/g, function (match) {
      if (split$2(value, ',').pop().indexOf('gradient(') > -1) {
        return match;
      }
...
function optimizeBody(rule, properties, context) {
  ...
  for (var i = 0, l = _properties.length; i < l; i++) {
    ...
    if (!PROPERTY_NAME_PATTERN.test(name)) {
      ...
      context.warnings.push('Invalid property name \'' + name + '\' at ' + formatPosition$2(propertyToken[1][2][0]) + '. Ignoring.');
      property.unused = true;
    }

    if (property.value.length === 0) {
      propertyToken = property.all[property.position];
      context.warnings.push('Empty property \'' + name + '\' at ' + formatPosition$2(propertyToken[1][2][0]) + '. Ignoring.');
      property.unused = true;
    }
    ...
    for (var j = 0, m = property.value.length; j < m; j++) {
      ...
      if (type == Token$l.PROPERTY_BLOCK) {
        property.unused = true;
        context.warnings.push('Invalid value token at ' + formatPosition$2(value[0][1][2][0]) + '. Ignoring.');
        break;
      }

      if (valueIsUrl && !context.validator.isUrl(value)) {
        property.unused = true;
        context.warnings.push('Broken URL \'' + value + '\' at ' + formatPosition$2(property.value[j][2][0]) + '. Ignoring.');
        break;
      }
...
function optimizeComment(token, options) {
  if (token[1][2] == Marker$9.EXCLAMATION && (options.level[OptimizationLevel$6.One].specialComments == 'all' || options.commentsKept < options.level[OptimizationLevel$6.One].specialComments)) {
...
function cleanupCharsets(tokens) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (hasCharset || token[1].indexOf(CHARSET_TOKEN) == -1) {
      ...
      hasCharset = true;
      tokens.splice(i, 1);
      tokens.unshift([Token$l.AT_RULE, token[1].replace(CHARSET_REGEXP, CHARSET_TOKEN)]);
    }
  }
}
...
function level1Optimize$1(tokens, context) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (token[0] == Token$l.COMMENT && token[1].length === 0 || levelOptions.removeEmpty && (token[1].length === 0 || (token[2] && token[2].length === 0))) {
      tokens.splice(i, 1);
      i--;
      l--;
    }
...
function isMergeable$4(selector, mergeablePseudoClasses, mergeablePseudoElements, multiplePseudoMerging) {
  var singleSelectors = split$1(selector, Marker$8.COMMA);
  var singleSelector;
  var i, l;
  ...
  for (i = 0, l = singleSelectors.length; i < l; i++) {
    ...
    if (singleSelector.length === 0 ||
        ...
        (singleSelector.indexOf(Marker$8.COLON) > -1 && !areMergeable(singleSelector, extractPseudoFrom(singleSelector), mergeablePseudoClasses, mergeablePseudoElements, multiplePseudoMerging))) {
      return false;
    }
  }
...
function extractPseudoFrom(selector) {
  ...
  for (index = 0, len = selector.length; index < len; index++) {
    ...
    } else if (character == Marker$8.DOUBLE_QUOTE && level == Level$1.ROOT) {
      buffer.push(character);
      level = Level$1.DOUBLE_QUOTE;
    } else if (character == Marker$8.DOUBLE_QUOTE && level == Level$1.DOUBLE_QUOTE) {
      buffer.push(character);
      level = Level$1.ROOT;
    } else if (character == Marker$8.SINGLE_QUOTE && level == Level$1.ROOT) {
      buffer.push(character);
      level = Level$1.SINGLE_QUOTE;
    } else if (character == Marker$8.SINGLE_QUOTE && level == Level$1.SINGLE_QUOTE) {
      buffer.push(character);
      level = Level$1.ROOT;
    } else if (isQuoted) {
      buffer.push(character);
    } else if (character == Marker$8.OPEN_ROUND_BRACKET) {
      buffer.push(character);
      roundBracketLevel++;
    } else if (character == Marker$8.CLOSE_ROUND_BRACKET && roundBracketLevel == 1 && isPseudo) {
      buffer.push(character);
      list.push(buffer.join(''));
      roundBracketLevel--;
      buffer = [];
      isPseudo = false;
    } else if (character == Marker$8.CLOSE_ROUND_BRACKET) {
      buffer.push(character);
      roundBracketLevel--;
    } else if (character == Marker$8.COLON && roundBracketLevel === 0 && isPseudo && !wasColon) {
      list.push(buffer.join(''));
      buffer = [];
      buffer.push(character);
    } else if (character == Marker$8.COLON && roundBracketLevel === 0 && !wasColon) {
      buffer = [];
      buffer.push(character);
      isPseudo = true;
    } else if (character == Marker$8.SPACE && roundBracketLevel === 0 && isPseudo) {
      list.push(buffer.join(''));
      buffer = [];
      isPseudo = false;
    ...
    isEscaped = character == Marker$8.BACK_SLASH;
    wasColon = character == Marker$8.COLON;
  }
...
function areAllowed(matches, mergeablePseudoClasses, mergeablePseudoElements) {
  ...
  for (i = 0, l = matches.length; i < l; i++) {
    match = matches[i];
    name = match.indexOf(Marker$8.OPEN_ROUND_BRACKET) > -1 ?
      match.substring(0, match.indexOf(Marker$8.OPEN_ROUND_BRACKET)) :
      match;
...
function needArguments(matches) {
  ...
  for (i = 0, l = matches.length; i < l; i++) {
    ...
    bracketOpensAt = match.indexOf(Marker$8.OPEN_ROUND_BRACKET);
  ...
  for (i = 0, l = matches.length; i < l; i++) {
    ...
    if (areChained) {
      name = match.indexOf(Marker$8.OPEN_ROUND_BRACKET) > -1 ?
        match.substring(0, match.indexOf(Marker$8.OPEN_ROUND_BRACKET)) :
        match;
      nextName = nextMatch.indexOf(Marker$8.OPEN_ROUND_BRACKET) > -1 ?
        nextMatch.substring(0, nextMatch.indexOf(Marker$8.OPEN_ROUND_BRACKET)) :
        nextMatch;
...
function everyValuesPair$2(fn, left, right) {
  ...
  for (position = 0; position < total; position++) {
    ...
    if (leftValue == Marker$7.COMMA || rightValue == Marker$7.COMMA) {
      continue;
    }
...
function _wrapDefault(name, property, compactable) {
  ...
  if (descriptor.doubleValues && descriptor.defaultValue.length == 2) {
    return wrapSingle$2([
      Token$k.PROPERTY,
      [Token$k.PROPERTY_NAME, name],
      [Token$k.PROPERTY_VALUE, descriptor.defaultValue[0]],
      [Token$k.PROPERTY_VALUE, descriptor.defaultValue[1]]
    ]);
    ...
    return wrapSingle$2([
      Token$k.PROPERTY,
      [Token$k.PROPERTY_NAME, name],
      [Token$k.PROPERTY_VALUE, descriptor.defaultValue]
    ]);
  }
}
...
function animation(property, compactable, validator) {
  ...
  if (values.length > 1 && _anyIsInherit(values)) {
    throw new InvalidPropertyError$1('Invalid animation values at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }
  ...
  for (i = 0, l = values.length; i < l; i++) {
    ...
    if (validator.isTime(value[1]) && !durationSet) {
      ...
      throw new InvalidPropertyError$1('Invalid animation value at ' + formatPosition$1(value[2][0]) + '. Ignoring.');
    }
  }
...
function background$1(property, compactable, validator) {
  ...
  for (var i = values.length - 1; i >= 0; i--) {
    ...
    if (validator.isBackgroundAttachmentKeyword(value[1])) {
      ...
      if (i > 0) {
        ...
        if (previousValue[1] == Marker$6.FORWARD_SLASH) {
          size.value = [value];
        } else if (i > 1 && values[i - 2][1] == Marker$6.FORWARD_SLASH) {
  ...
  if (!anyValueSet) {
    throw new InvalidPropertyError$1('Invalid background value at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }
...
function borderRadius$1(property, compactable) {
  ...
  for (var i = 0, l = values.length; i < l; i++) {
    if (values[i][1] == Marker$6.FORWARD_SLASH) {
      splitAt = i;
      break;
    }
  }
  ...
  if (splitAt === 0 || splitAt === values.length - 1) {
    throw new InvalidPropertyError$1('Invalid border-radius value at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }
...
function font$1(property, compactable, validator) {
  ...
  if (!values[index]) {
    throw new InvalidPropertyError$1('Missing font values at ' + formatPosition$1(property.all[property.position][1][2][0]) + '. Ignoring.');
  }
  ...
  if (values.length == 1 && (validator.isFontKeyword(values[0][1]) || validator.isGlobal(values[0][1]) || validator.isPrefixed(values[0][1]))) {
    values[0][1] = Marker$6.INTERNAL + values[0][1];
    style.value = variant.value = weight.value = stretch.value = size.value = height.value = family.value = values;
    return components;
  }

  if (values.length < 2 || !_anyIsFontSize(values, validator) || !_anyIsFontFamily(values, validator)) {
    throw new InvalidPropertyError$1('Invalid font values at ' + formatPosition$1(property.all[property.position][1][2][0]) + '. Ignoring.');
  }

  if (values.length > 1 && _anyIsInherit(values)) {
    throw new InvalidPropertyError$1('Invalid font values at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }
  ...
  while (index < fuzzyMatched) {
    ...
    if (isStyleValid && !isStyleSet) {
      ...
      throw new InvalidPropertyError$1('Invalid font style / variant / weight / stretch value at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
    } else {
      break;
    }
  ...
  if (validator.isFontSizeKeyword(values[index][1]) || validator.isUnit(values[index][1]) && !validator.isDynamicUnit(values[index][1])) {
    ...
    throw new InvalidPropertyError$1('Missing font size at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }

  if (!values[index]) {
    throw new InvalidPropertyError$1('Missing font family at ' + formatPosition$1(values[0][2][0]) + '. Ignoring.');
  }

  // ... and perhaps line-height
  if (isSizeSet && values[index] && values[index][1] == Marker$6.FORWARD_SLASH && values[index + 1] && (validator.isLineHeightKeyword(values[index + 1][1]) || validator.isUnit(values[index + 1][1]) || validator.isNumber(values[index + 1][1]))) {
  ...
  while (values[index]) {
    if (values[index][1] == Marker$6.COMMA) {
      appendableFamilyName = false;
    } else {
      if (appendableFamilyName) {
        family.value[family.value.length - 1][1] += Marker$6.SPACE + values[index][1];
      } else {
...
function fourValues$1(property, compactable) {
  ...
  for (var i = componentNames.length - 1; i >= 0; i--) {
    var component = wrapSingle$2([
      Token$k.PROPERTY,
      [Token$k.PROPERTY_NAME, componentNames[i]]
    ]);
  ...
  return function (property, compactable, validator) {
    ...
    for (i = 0, l = components.length; i < l; i++) {
      ...
      for (j = 1, m = splitComponents.length; j < m; j++) {
        components[i].value.push([Token$k.PROPERTY_VALUE, Marker$6.COMMA]);
...
function shallow(property) {
  var cloned = wrapSingle$1([
    Token$j.PROPERTY,
    [Token$j.PROPERTY_NAME, property.name]
  ]);
...
function isInheritOnly(values) {
  for (var i = 0, l = values.length; i < l; i++) {
    ...
    if (value != 'inherit' && value != Marker$5.COMMA && value != Marker$5.FORWARD_SLASH)
      return false;
  }
...
function background(property, compactable, lastInMultiplex) {
  ...
  for (var i = components.length - 1; i >= 0; i--) {
    ...
    if (component.name == 'background-clip') {
      ...
      if (needsOne) {
        ...
        restoreValue(component);
        restored.unshift([Token$i.PROPERTY_VALUE, Marker$5.FORWARD_SLASH]);
        restoreValue(positionComponent);
  ...
  if (restored.length === 0)
    restored.push([Token$i.PROPERTY_VALUE, compactable[property.name].defaultValue]);
...
function borderRadius(property, compactable) {
  if (property.multiplex) {
    ...
    if (horizontalValues.length == verticalValues.length &&
      ...
      return horizontalValues.concat([[Token$i.PROPERTY_VALUE, Marker$5.FORWARD_SLASH]]).concat(verticalValues);
    }
  } else {
...
function font(property, compactable) {
  ...
  if (property.value[0][1].indexOf(Marker$5.INTERNAL) === 0) {
    property.value[0][1] = property.value[0][1].substring(Marker$5.INTERNAL.length);
  ...
  if (components[componentIndex].value[0][1] != compactable[components[componentIndex].name].defaultValue) {
    Array.prototype.push.apply(restored, [[Token$i.PROPERTY_VALUE, Marker$5.FORWARD_SLASH]]);
    Array.prototype.push.apply(restored, components[componentIndex].value);
  }
  ...
  while (components[componentIndex].value[fontFamilyIndex]) {
    ...
    if (components[componentIndex].value[fontFamilyIndex + 1]) {
      restored.push([Token$i.PROPERTY_VALUE, Marker$5.COMMA]);
    }
...
function multiplex(restoreWith) {
  return function (property, compactable) {
    ...
    for (i = 0, l = property.components[0].value.length; i < l; i++) {
      if (property.components[0].value[i][1] == Marker$5.COMMA)
        multiplexSize++;
    }
    ...
    for (i = 0; i <= multiplexSize; i++) {
      ...
      for (var j = 0, m = property.components.length; j < m; j++) {
        ...
        for (var k = componentMultiplexSoFar[_component.name] || 0, n = componentToClone.value.length; k < n; k++) {
          if (componentToClone.value[k][1] == Marker$5.COMMA) {
...
function cloneDescriptor(propertyName, prefix) {
  var clonedDescriptor = override$3(compactable$7[propertyName], {});
...
for (var propertyName in compactable$7) {
  var descriptor = compactable$7[propertyName];
...
var compactable_1 = override$3(compactable$7, vendorPrefixedCompactable);

var compactable$6 = compactable_1;
...
function populateComponents$3(properties, validator, warnings) {
  ...
  for (var i = properties.length - 1; i >= 0; i--) {
    var property = properties[i];
    var descriptor = compactable$6[property.name];
    ...
    if (descriptor && descriptor.shorthand) {
      ...
      try {
        property.components = descriptor.breakUp(property, compactable$6, validator);

        if (descriptor.shorthandComponents) {
          for (j = 0, m = property.components.length; j < m; j++) {
            component = property.components[j];
            component.components = compactable$6[component.name].breakUp(component, compactable$6, validator);
          }
        }
      } catch (e) {
...
var compactable$5 = compactable_1;

function restoreWithComponents$3(property) {
  var descriptor = compactable$5[property.name];

  if (descriptor && descriptor.shorthand) {
    return descriptor.restore(property, compactable$5);
  } else {
    return property.value;
  }
}
...
var compactable$4 = compactable_1;
...
function mergeIntoShorthands$1(properties, validator) {
  ...
  for (i = 0, l = properties.length; i < l; i++) {
    property = properties[i];
    descriptor = compactable$4[property.name];
...
function invalidateOrCompact(properties, position, candidates, validator) {
  ...
  for (shorthandName in candidates) {
    ...
    shorthandDescriptor = compactable$4[shorthandName];
...
function invalidates(candidates, shorthandName, invalidatedBy) {
  var shorthandDescriptor = compactable$4[shorthandName];
  var invalidatedByDescriptor = compactable$4[invalidatedBy.name];
  var componentName;
...
function overridable(components, shorthandName, validator) {
  var descriptor = compactable$4[shorthandName];
  ...
  for (i = 0, l = descriptor.components.length; i < l; i++) {
    ...
    mayOverride = compactable$4[component.name].canOverride;
...
function mergeable(components) {
  ...
  for (componentName in components) {
    ...
    descriptor = compactable$4[componentName];
    ...
    values = descriptor.restore(component, compactable$4);
...
function inferComponentValue(components, propertyName) {
  var descriptor = compactable$4[propertyName];
...
var compactable$3 = compactable_1;
...
function findInSubComponents(shorthand, comparator) {
  ...
  if (!compactable$3[shorthand.name].shorthandComponents) {
    return;
  }
...
var compactable$2 = compactable_1;

function isComponentOf$1(property1, property2, shallow) {
  return isDirectComponentOf(property1, property2) ||
    !shallow && !!compactable$2[property1.name].shorthandComponents && isSubComponentOf(property1, property2);
}

function isDirectComponentOf(property1, property2) {
  var descriptor = compactable$2[property1.name];
...
var compactable$1 = compactable_1;

function overridesNonComponentShorthand$1(property1, property2) {
  return property1.name in compactable$1 &&
    'overridesShorthands' in compactable$1[property1.name] &&
    compactable$1[property1.name].overridesShorthands.indexOf(property2.name) > -1;
}
...
var http = require$$0__default$1;
var https = require$$1__default;
var url$4 = require$$0__default$2;
...
var url$3 = require$$0__default$2;
...
function rebaseFrom$1(rebaseOption) {
...
var url$2 = require$$0__default$2;
...
var url$1 = require$$0__default$2;
...
function isDataUriResource$1(uri) {
  return DATA_URI_PATTERN.test(uri);
}

var isDataUriResource_1 = isDataUriResource$1;
...
function findTokenSource(token) {
  ...
  if (token[0] == Token$3.AT_RULE || token[0] == Token$3.COMMENT) {
    metadata = token[2][0];
  } else {
    scope = token[1][0];
    metadata = scope[2][0];
...
function intoTokens(source, externalContext, internalContext, isNested) {
  ...
  for (; position.index < source.length; position.index++) {
    ...
    if (isEscaped) {
      ...
      level = levels.pop();
      buffer.push(character);
    } else if (!isCommentStart && !isCommentEnd && character != Marker$1.CLOSE_ROUND_BRACKET && character != Marker$1.OPEN_ROUND_BRACKET && level != Level.COMMENT && !isQuoted && roundBracketLevel > 0) {
      ...
      seekingValue = true;
      buffer = [];
    } else if (character == Marker$1.SEMICOLON && level == Level.RULE && propertyToken && ruleTokens.length > 0 && buffer.length > 0 && buffer[0] == Marker$1.AT) {
      ...
      buffer = [];
    } else if (character == Marker$1.SEMICOLON && level == Level.RULE && propertyToken && buffer.length > 0) {
      ...
      propertyToken = null;
      seekingValue = false;
    } else if (character == Marker$1.SEMICOLON && level == Level.RULE && buffer.length > 0 && buffer[0] == Marker$1.AT) {
      ...
      buffer = [];
    } else if (character == Marker$1.SEMICOLON && level == Level.RULE && buffer.length === 0) ; else if (character == Marker$1.CLOSE_CURLY_BRACKET && level == Level.RULE && propertyToken && seekingValue && buffer.length > 0 && ruleTokens.length > 0) {
      ...
      seekingValue = false;
      buffer = [];
    } else if (character == Marker$1.CLOSE_CURLY_BRACKET && level == Level.RULE && propertyToken && buffer.length > 0 && buffer[0] == Marker$1.AT && ruleTokens.length > 0) {
      ...
      level = levels.pop();
      seekingValue = false;
    } else if (character == Marker$1.CLOSE_CURLY_BRACKET && level == Level.RULE && propertyToken && buffer.length > 0) {
      ...
      seekingValue = false;
      buffer = [];
    } else if (character == Marker$1.CLOSE_CURLY_BRACKET && level == Level.RULE && buffer.length > 0 && buffer[0] == Marker$1.AT) {
      ...
      buffer.push(character);
      roundBracketLevel--;
    } else if (character == Marker$1.FORWARD_SLASH && source[position.index + 1] != Marker$1.ASTERISK && level == Level.RULE && seekingValue && buffer.length > 0) {
      ...
      buffer = [];
    } else if (character == Marker$1.COMMA && level == Level.RULE && seekingValue && buffer.length > 0) {
      ...
      buffer = [];
    } else if (character == Marker$1.CLOSE_SQUARE_BRACKET && propertyToken && propertyToken.length > 1 && buffer.length > 0 && isRepeatToken(buffer)) {
      buffer.push(character);
      serializedBuffer = buffer.join('').trim();
...
function optimize(tokens, context) {
  ...
  optimized = level0Optimize(tokens);
...
exports.index = index;
//# sourceMappingURL=dep-3b2c4440.js.map
