...
var build = require('./chunks/dep-bc4956ea.js');
...
async function preview(config, port = 5000) {
    ...
    httpServer.listen(port, hostname, () => {
        ...
        build.printServerUrls(hostname, protocol, port, base, logger.info);
        if (options.open) {
            const path = typeof options.open === 'string' ? options.open : base;
            build.openBrowser(`${protocol}://${hostname}:${port}${path}`, true, logger);
        }
    });
}
...
cli
    ...
    // is ok here
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/dep-bc4956ea.js'); }).then(function (n) { return n.index$1; });
    try {
...
cli
    ...
    .action(async (root, options) => {
    const { build: build$1 } = await Promise.resolve().then(function () { return require('./chunks/dep-bc4956ea.js'); }).then(function (n) { return n.build$1; });
...
cli
    ...
    .option('--force', `[boolean] force the optimizer to ignore the cache and re-bundle`)
    .action(async (root, options) => {
    const { optimizeDeps } = await Promise.resolve().then(function () { return require('./chunks/dep-bc4956ea.js'); }).then(function (n) { return n.index; });
    try {
